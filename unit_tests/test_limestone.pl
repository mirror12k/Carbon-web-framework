...
sub test_store {
	my $success = 1;
	my ($res, $req);
	my $test_name = 'test store';

	# clear the test directory in case it's left over from the last time
	rmtree('test_limestone_test_store');
	...
	my $thr = threads->create(sub {
		my $svr = Carbon::Limestone->new(
			...
			ssl_key => 'test_ssl_test_basic/key.pem',
			port => 2049,
			database_filepath => 'test_limestone_test_store' );
	...
	$thr = threads->create(sub {
		my $svr = Carbon::Limestone->new(
			ssl_certificate => 'test_ssl_test_basic/cert.pem', # cheat by using the test_ssl certificate
			ssl_key => 'test_ssl_test_basic/key.pem',
			port => 2050, # currently cheating by changing the port number, TOFIX
			database_filepath => 'test_limestone_test_store' );
	...
	$client = Carbon::Limestone::Client->new(hostport => 'localhost:2050', username => 'root', password => 'root');
	$table = $client->client('Limestone::Table=test_table');
